...
def render_debug_table(associated_ap: str = "lobby") -> str:
    return render_table(
        [
            "MAC",
            "IP Address",
...
DEBUG_TABLE = render_debug_table()
...
def snapshot_outputs(*, associated_ap: str = "192.0.2.10") -> dict[str, str]:
    debug_clients = render_debug_table(associated_ap)
    return {
        "show aps": command_output("show aps", f"2 Access Points\n{APS_TABLE}"),
        "show client debug": command_output("show client debug", debug_clients),
...
def test_snapshot_client_count_comes_from_show_summary() -> None:
...
def test_snapshot_does_not_derive_missing_client_count_from_rows() -> None:
    outputs = snapshot_outputs()
    outputs["show summary"] = command_output(
    ...
    assert snapshot.cluster.client_count is None
